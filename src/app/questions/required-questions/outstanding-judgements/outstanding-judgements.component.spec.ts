...
import { QuestionsRequiredQuestionsOutstandingJudgementsComponent } from './outstanding-judgements.component';
import {
...
describe('QuestionsRequiredQuestionsOutstandingJudgementsComponent', () => {
  let component: QuestionsRequiredQuestionsOutstandingJudgementsComponent;
  let fixture: ComponentFixture<QuestionsRequiredQuestionsOutstandingJudgementsComponent>;
  let activatedRoute;
  let router;
  let store;
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(QuestionsRequiredQuestionsOutstandingJudgementsComponent);
  ...
  describe('the component form', () => {
    it('should have the outstandingJudgements control with no default value', () => {
      expect(component.form.controls['outstandingJudgements']).toBeDefined();
      expect(component.form.controls['outstandingJudgements'].value).toEqual(null);
    });

    it('should require the outstandingJudgements control', () => {
      component.form.controls['outstandingJudgements'].setValue(null);
      expect(component.form.valid).toBeFalsy();

      component.form.controls['outstandingJudgements'].setValue('outstandingJudgements');
      expect(component.form.valid).toBeTruthy();
    });

    describe('with previous data in the store', () => {
      it('should set the value of the control to "yes" if the store is true OnInit', () => {
        store.dispatch(new GovernmentQuestionsOutstandingJudgementsUpdateAction(true));
        component.ngOnInit();
        expect(component.form.controls['outstandingJudgements'].value).toEqual('yes');
      });

      it('should set the value of the control to "no" if the store is false OnInit', () => {
        store.dispatch(new GovernmentQuestionsOutstandingJudgementsUpdateAction(false));
        component.ngOnInit();
        expect(component.form.controls['outstandingJudgements'].value).toEqual('no');
      });
  ...
  describe('setting up onSubmitMethod', () => {
    beforeEach(() => {
      component.form = ({
        valid: true,
        value: {
          outstandingJudgements: 'yes'
        }
      } as FormGroup);
      component.onSubmit();
    });

    it('should dispatch an event to update the store sending in a true when outstandingJudgements is yes', () => {
      expect(store.dispatch).toHaveBeenCalledWith(new GovernmentQuestionsOutstandingJudgementsUpdateAction(
        true
      ));
    });

    it('should dispatch an event to update the store sending in a false when outstandingJudgements is no', () => {
  ...
  describe('HTML integration tests', () => {
    it('should map the #yes input to the outstandingJudgements control', () => {
    ...
    it('should map the #no input to the outstandingJudgements control', () => {
