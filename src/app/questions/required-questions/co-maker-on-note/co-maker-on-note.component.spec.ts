...
import { QuestionsRequiredQuestionsCoMakerOnNoteComponent } from './co-maker-on-note.component';
import {
...
describe('QuestionsRequiredQuestionsCoMakerOnNoteComponent', () => {
  let component: QuestionsRequiredQuestionsCoMakerOnNoteComponent;
  let fixture: ComponentFixture<QuestionsRequiredQuestionsCoMakerOnNoteComponent>;
  let activatedRoute;
  let router;
  let store;
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(QuestionsRequiredQuestionsCoMakerOnNoteComponent);
  ...
  describe('the component form', () => {
    it('should have the coMakerOnNote control with no default value', () => {
      expect(component.form.controls['coMakerOnNote']).toBeDefined();
      expect(component.form.controls['coMakerOnNote'].value).toEqual(null);
    });

    it('should require the coMakerOnNote control', () => {
      component.form.controls['coMakerOnNote'].setValue(null);
      expect(component.form.valid).toBeFalsy();

      component.form.controls['coMakerOnNote'].setValue('coMakerOnNote');
      expect(component.form.valid).toBeTruthy();
    });

    describe('with previous data in the store', () => {
      it('should set the value of the control to "yes" if the store is true OnInit', () => {
        store.dispatch(new GovernmentQuestionsCoMakerOnNoteUpdateAction(true));
        component.ngOnInit();
        expect(component.form.controls['coMakerOnNote'].value).toEqual('yes');
      });

      it('should set the value of the control to "no" if the store is false OnInit', () => {
        store.dispatch(new GovernmentQuestionsCoMakerOnNoteUpdateAction(false));
        component.ngOnInit();
        expect(component.form.controls['coMakerOnNote'].value).toEqual('no');
      });
  ...
  describe('setting up onSubmitMethod', () => {
    ...
    it('should dispatch an event to update the store sending in a true when coMakerOnNote is yes', () => {
      expect(store.dispatch).toHaveBeenCalledWith(new GovernmentQuestionsCoMakerOnNoteUpdateAction(
        true
      ));
    });

    it('should dispatch an event to update the store sending in a false when coMakerOnNote is no', () => {
  ...
  describe('HTML integration tests', () => {
    it('should map the #yes input to the coMakerOnNote control', () => {
    ...
    it('should map the #no input to the coMakerOnNote control', () => {
