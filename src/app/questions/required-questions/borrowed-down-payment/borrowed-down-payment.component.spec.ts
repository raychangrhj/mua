...
import { QuestionsRequiredQuestionsBorrowedDownPaymentComponent } from './borrowed-down-payment.component';
import {
...
describe('QuestionsRequiredQuestionsBorrowedDownPaymentComponent', () => {
  let component: QuestionsRequiredQuestionsBorrowedDownPaymentComponent;
  let fixture: ComponentFixture<QuestionsRequiredQuestionsBorrowedDownPaymentComponent>;
  let activatedRoute;
  let router;
  let store;
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(QuestionsRequiredQuestionsBorrowedDownPaymentComponent);
  ...
  describe('the component form', () => {
    it('should have the borrowedDownPayment control with no default value', () => {
      expect(component.form.controls['borrowedDownPayment']).toBeDefined();
      expect(component.form.controls['borrowedDownPayment'].value).toEqual(null);
    });

    it('should require the borrowedDownPayment control', () => {
      component.form.controls['borrowedDownPayment'].setValue(null);
      expect(component.form.valid).toBeFalsy();

      component.form.controls['borrowedDownPayment'].setValue('borrowedDownPayment');
      expect(component.form.valid).toBeTruthy();
    });

    describe('with previous data in the store', () => {
      it('should set the value of the control to "yes" if the store is true OnInit', () => {
        store.dispatch(new GovernmentQuestionsBorrowedDownPaymentUpdateAction(true));
        component.ngOnInit();
        expect(component.form.controls['borrowedDownPayment'].value).toEqual('yes');
      });

      it('should set the value of the control to "no" if the store is false OnInit', () => {
        store.dispatch(new GovernmentQuestionsBorrowedDownPaymentUpdateAction(false));
        component.ngOnInit();
        expect(component.form.controls['borrowedDownPayment'].value).toEqual('no');
      });
  ...
  describe('setting up onSubmitMethod', () => {
    beforeEach(() => {
      component.form = ({
        valid: true,
        value: {
          borrowedDownPayment: 'yes'
        }
      } as FormGroup);
      component.onSubmit();
    });

    it('should dispatch an event to update the store sending in a true when borrowedDownPayment is yes', () => {
      expect(store.dispatch).toHaveBeenCalledWith(new GovernmentQuestionsBorrowedDownPaymentUpdateAction(
        true
      ));
    });

    it('should dispatch an event to update the store sending in a false when borrowedDownPayment is no', () => {
  ...
  describe('HTML integration tests', () => {
    it('should map the #yes input to the borrowedDownPayment control', () => {
    ...
    it('should map the #no input to the borrowedDownPayment control', () => {
