...
import { QuestionsRequiredQuestionsUsCitizenComponent } from './us-citizen.component';
import {
...
describe('QuestionsRequiredQuestionsUsCitizenComponent', () => {
  let component: QuestionsRequiredQuestionsUsCitizenComponent;
  let fixture: ComponentFixture<QuestionsRequiredQuestionsUsCitizenComponent>;
  let activatedRoute;
  let router;
  let store;
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(QuestionsRequiredQuestionsUsCitizenComponent);
  ...
  describe('the component form', () => {
    it('should have the usCitizen control with no default value', () => {
      expect(component.form.controls['usCitizen']).toBeDefined();
      expect(component.form.controls['usCitizen'].value).toEqual(null);
    });

    it('should require the usCitizen control', () => {
      component.form.controls['usCitizen'].setValue(null);
      expect(component.form.valid).toBeFalsy();

      component.form.controls['usCitizen'].setValue('usCitizen');
      expect(component.form.valid).toBeTruthy();
    });

    describe('with previous data in the store', () => {
      it('should set the value of the control to "yes" if the store is true OnInit', () => {
        store.dispatch(new GovernmentQuestionsUSCitizenUpdateAction(true));
        component.ngOnInit();
        expect(component.form.controls['usCitizen'].value).toEqual('yes');
      });

      it('should set the value of the control to "no" if the store is false OnInit', () => {
        store.dispatch(new GovernmentQuestionsUSCitizenUpdateAction(false));
        component.ngOnInit();
        expect(component.form.controls['usCitizen'].value).toEqual('no');
      });
  ...
  describe('setting up onSubmitMethod', () => {
    ...
    it('should dispatch an event to update the store sending in a true when usCitizen is yes', () => {
      expect(store.dispatch).toHaveBeenCalledWith(new GovernmentQuestionsUSCitizenUpdateAction(
        true
      ));
    });

    it('should dispatch an event to update the store sending in a false when usCitizen is no', () => {
  ...
  describe('HTML integration tests', () => {
    it('should map the #yes input to the usCitizen control', () => {
    ...
    it('should map the #no input to the usCitizen control', () => {
